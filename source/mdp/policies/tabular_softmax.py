...
class TabularSoftmax(Policy):
    ...
    def parameters(self, p:np.ndarray):
        """
        Update the policy parameters. Input is a 1D numpy array of siactionactionze |S|x|A|.
        """
    ...
    def sampleAction(self, state:int)->int:
        ...
        actions = np.arange(self._theta.shape[1])
    ...
    def getActionProbabilities(self, state:int)->np.ndarray:
        ...
        output:
            distribution -- a 1D numpy array representing a probability 
                            ...
                            the state provided.
        """
        den = np.sum(np.exp(self._sigma * self._theta[state]))
        nums = np.exp(self._sigma * self._theta[state])
        # print(nums / den)
        return nums / den